...
With your SESSION_ID ready, insert it into the command below, replacing `<your_session_id>` with your actual session ID. Then, run the command to start the Nestri server, also replace `<your-nestri-image>` with your build nestri image or nestri or pulled remote image
